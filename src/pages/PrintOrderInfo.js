...
const ModalConfirmDelete = ({ visible, cancel, data, onSelectItem }) => {
  ...
  const handleItemPress = (item) => {
    Alert.alert("Дэлгүүр сонголт", ` ${item.storename} дэлгүүрийг сонгох уу?`, [
      {
        text: "No",
        style: "cancel",
      },
            ...
            renderItem={({ item, index }) => (
              ...
              >
                <Text style={styles.textlist}>
                  {parseInt(index) + 1} | {item.storename} | {item.sumtotal}{" "}
                  (Нийт дүн)- {item.cash === null ? 0 : item.cash} (бэлэн) ={" "}
                  {item.sumtotal - item.cash} (үлдэгдэл)
                </Text>
